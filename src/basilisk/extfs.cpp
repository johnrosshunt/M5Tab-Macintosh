...
// Runtime-visible trace for ExtFS errors. We keep DEBUG=0 so the firehose
// D(bug()) prints stay off, but error paths (open/stat/read/write/seek
// failures) emit a short [EXTFS] line over the main serial console so a
// "disk error" dialog on the Mac side can be matched to a specific POSIX
// errno + filename. Set BASILISK_EXTFS_TRACE=0 via build flags to silence.
#ifndef BASILISK_EXTFS_TRACE
#define BASILISK_EXTFS_TRACE 1
#endif
#if BASILISK_EXTFS_TRACE && defined(ARDUINO)
#include <Arduino.h>
#define EXTFS_ERR(...)                                                        \
	do {                                                                      \
		Serial.printf("[EXTFS] " __VA_ARGS__);                                \
	} while (0)
#else
#define EXTFS_ERR(...) ((void)0)
#endif
...
void ExtFSInit(void)
{
	...
	*RootPath = 0;
	...
	if (path == NULL) {
		EXTFS_ERR("init: no extfs pref, shared folder disabled\n");
		return;
	}
	strncpy(RootPath, path, MAX_PATH_LENGTH - 1);
	RootPath[MAX_PATH_LENGTH - 1] = 0;
	if (stat(RootPath, &root_stat)) {
		EXTFS_ERR("init: stat(%s) failed, errno=%d\n", RootPath, errno);
		return;
	}
	if (!S_ISDIR(root_stat.st_mode)) {
		EXTFS_ERR("init: %s is not a directory\n", RootPath);
		return;
	}
	ready = true;
	EXTFS_ERR("init ok: root=%s\n", RootPath);
	{

		// Override VOLUME_NAME with the basename of the selected host
		// folder so the Mac desktop shows a meaningful label (e.g.
		// "/sd/Shared" -> "Shared") instead of the generic "Host"
		// default from STR_EXTFS_VOLUME_NAME. If the selected path is
		// the VFS mount root itself (no meaningful basename) we fall
		// back to "SD Card". The stored pstring is length-prefixed and
		// capped at 27 bytes of content to fit the HFS volume name
		// field.
		const char *slash = strrchr(RootPath, '/');
		const char *base = (slash != NULL) ? slash + 1 : RootPath;
		char label[28];
		if (base[0] == '\0') {
		...
		cstr2pstr(VOLUME_NAME, label);
		EXTFS_ERR("init: volume label = \"%s\"\n", label);

		// The root FSItem's guest_name drives a few trap paths that
		// look at the volume entry itself. Keep it in sync with the
		// freshly computed volume label so the Finder and the VCB
		// agree on how the drive is named.
		FSItem *root_item = find_fsitem_by_id(ROOT_ID);
		if (root_item != NULL) {
			strncpy(root_item->guest_name, label,
			        sizeof(root_item->guest_name) - 1);
			root_item->guest_name[sizeof(root_item->guest_name) - 1] = '\0';
		}
	}
}
...
static int16 fs_open(uint32 pb, uint32 dirID, uint32 vcb, bool resource_fork)
{
	...
	if (resource_fork) {
		if (access(full_path, F_OK)) {
			EXTFS_ERR("rfork access fail: %s errno=%d\n", full_path, errno);
			return fnfErr;
		}
		fd = open_rfork(full_path, flag);
		if (fd >= 0) {
			if (fstat(fd, &st) < 0) {
				EXTFS_ERR("rfork fstat fail: %s errno=%d\n", full_path, errno);
				close(fd);
				return errno2oserr();
			}
		...
		if (fd < 0) {
			EXTFS_ERR("open fail: %s flag=0x%x errno=%d\n",
			          full_path, flag, errno);
			return errno2oserr();
		}
		if (fstat(fd, &st) < 0) {
			EXTFS_ERR("fstat fail: %s errno=%d\n", full_path, errno);
			close(fd);
			return errno2oserr();
		}
		EXTFS_ERR("open ok: %s fd=%d size=%ld flag=0x%x\n",
		          full_path, fd, (long)st.st_size, flag);
	}
...
static int16 fs_read(uint32 pb)
{
	...
	switch (ReadMacInt16(pb + ioPosMode) & 3) {
		case fsFromStart:
			if (lseek(fd, ReadMacInt32(pb + ioPosOffset), SEEK_SET) < 0) {
				EXTFS_ERR("read lseek SET fd=%d off=%u errno=%d\n",
				          fd, ReadMacInt32(pb + ioPosOffset), errno);
				return posErr;
			}
			break;
		case fsFromLEOF:
			if (lseek(fd, (int32)ReadMacInt32(pb + ioPosOffset), SEEK_END) < 0) {
				EXTFS_ERR("read lseek END fd=%d off=%d errno=%d\n",
				          fd, (int32)ReadMacInt32(pb + ioPosOffset), errno);
				return posErr;
			}
			break;
		case fsFromMark:
			if (lseek(fd, (int32)ReadMacInt32(pb + ioPosOffset), SEEK_CUR) < 0) {
				EXTFS_ERR("read lseek CUR fd=%d off=%d errno=%d\n",
				          fd, (int32)ReadMacInt32(pb + ioPosOffset), errno);
				return posErr;
			}
			break;
	...
	// Read
	ssize_t actual = extfs_read(fd, Mac2HostAddr(ReadMacInt32(pb + ioBuffer)), req_count);
	...
	WriteMacInt32(fcb + fcbCrPs, pos);
	WriteMacInt32(pb + ioPosOffset, pos);
	if (actual != (ssize_t)req_count) {
		if (actual < 0) {
			EXTFS_ERR("read fail fd=%d req=%u actual=-1 errno=%d oserr=%d\n",
			          fd, req_count, errno, read_err);
			return read_err;
		}
		// Short read = EOF; this is normal for the end of small files.
		return eofErr;
	} else {
		return noErr;
	}
