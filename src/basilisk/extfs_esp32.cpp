/*
 *  extfs_esp32.cpp - MacOS ExtFS host backend for ESP32 (SD card via VFS)
 *
 *  Adapted from extfs_unix.cpp in Basilisk II (C) 1997-2008 Christian Bauer.
 *
 *  ESP-IDF exposes SD (FAT/exFAT) through the POSIX VFS layer, so the Unix
 *  backend works almost verbatim. Finder info and resource forks are kept
 *  in .finf/ and .rsrc/ sidecar directories next to each file since FAT has
 *  no xattr support - this matches how the upstream Unix port behaves when
 *  built without ENABLE_XATTR_FINDER_INFO.
 *
 *  The extfs pref is expected to already be a full VFS path (e.g. "/Shared"
 *  on Tab5 where the SD is mounted at "/", or "/sd/Shared" on Waveshare
 *  where it's mounted at "/sd"). prefs_esp32.cpp prepends BOARD_SD_MOUNT_POINT
 *  before calling PrefsReplaceString("extfs", ...).
 *
